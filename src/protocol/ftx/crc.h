...
// Compute 14-bit CRC for a sequence of given number of bits using FT2/FT4/FT8 CRC polynomial
// [IN] message  - byte sequence (MSB first)
// [IN] num_bits - number of bits in the sequence
uint16_t ftx_compute_crc(const uint8_t message[], int num_bits);

/// Extract the FT2/FT4/FT8 CRC of a packed message (during decoding)
/// @param[in] a91 77 bits of payload data + CRC
/// @return Extracted CRC
uint16_t ftx_extract_crc(const uint8_t a91[]);

/// Add FT2/FT4/FT8 CRC to a packed message (during encoding)
